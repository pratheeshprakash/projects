...
{
Mat original = imread("sudoku.jpg", CV_LOAD_IMAGE_GRAYSCALE);
Mat img = imread("sudoku.jpg", CV_LOAD_IMAGE_GRAYSCALE);;
if (img.empty())
    {
...
Mat outerBox = Mat(img.size(), CV_8UC1);// new blank single channel image 
adaptiveThreshold(img, outerBox, 255, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY, 5, 2);
bitwise_not(outerBox, outerBox);
uchar kerneldata[]={ 0,1,0,1,1,1,0,1,0};
Mat kernel=Mat(3,3,CV_8UC1,kerneldata).clone();
 
//= (Mat_<uchar>(3,3) << 0,1,0,1,1,1,0,1,0);// kernel matrix for diallation
//imshow("Before Dialation", outerBox);
dilate(outerBox, outerBox, kernel);
	
// find biggest blob by flooding with gray
    int count=0;
    int max=-1;
    Point maxPt;
...
// flood remaining region with black
 for(int y=0;y<outerBox.size().height;y++)
    {
        uchar *row = outerBox.ptr(y);
        for(int x=0;x<outerBox.size().width;x++)
...
//merge line
mergeRelatedLines(&lines, img); // Add this line
...
//detect intersection point of these lines
...
// correct the skewness of the image
...
imshow("undistorted thresh",undistortedThreshed);


DigitRecognizer *dr = new DigitRecognizer();
bool b = dr->train("E:/open CV/projects/sudoku_solver/train-images.idx3-ubyte", "E:/open CV/projects/sudoku_solver/train-labels.idx1-ubyte");
printf("Trained: %d\n", b);
